...
obj.lastActive=+new Date();obj.pageX=e.pageX;obj.pageY=e.pageY;obj.tId=setTimeout(toggleIdleState,obj.timeout);},reset=function(){var obj=$.data(elem,"idleTimerObj")||{};obj.idle=obj.idleBackup;obj.olddate=+new Date();obj.lastActive=obj.olddate;obj.remaining=null;clearTimeout(obj.tId);if(!obj.idle){obj.tId=setTimeout(toggleIdleState,obj.timeout);}},pause=function(){var obj=$.data(elem,"idleTimerObj")||{};if(obj.remaining!=null){return;}
...
jqElem.on($.trim((opts.events+" ").split(" ").join("._idleTimer ")),function(e){handleEvent(e);});obj=$.extend({},{olddate:+new Date(),lastActive:+new Date(),idle:opts.idle,idleBackup:opts.idle,timeout:opts.timeout,remaining:null,tId:null,pageX:null,pageY:null});if(!obj.idle){obj.tId=setTimeout(toggleIdleState,obj.timeout);}
