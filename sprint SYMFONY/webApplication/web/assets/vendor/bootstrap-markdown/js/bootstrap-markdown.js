...
if(this.$element.is(':disabled')||this.$element.is('[readonly]')){this.disableButtons('all');}
...
return false;})}})}}else{this.$editor.show()}
if(options.autofocus){this.$textarea.focus()
this.$editor.addClass('active')}
if(options.initialstate==='preview'){this.showPreview();}
this.hideButtons(options.hiddenButtons)
this.disableButtons(options.disabledButtons)
options.onShow(this)
return this},parseContent:function(){var content,callbackContent=this.$options.onPreview(this)
...
if(link!=null){e.replaceSelection('!['+chunk+']('+link+' "'+e.__localize('enter image title here')+'")')
cursor=selected.start+2
e.setNextTab(e.__localize('enter image title here'))
e.setSelection(cursor,cursor+chunk.length)}}}]},{name:'groupMisc',data:[{name:'cmdList',hotkey:'Ctrl+U',title:'Unordered List',icon:{glyph:'glyphicon glyphicon-list',fa:'fa fa-list','fa-3':'icon-list-ul'},callback:function(e){var chunk,cursor,selected=e.getSelection(),content=e.getContent()
...
e.replaceSelection('\n\n'+list.join('\n'))
cursor=selected.start+4}}
e.setSelection(cursor,cursor+chunk.length)}},{name:'cmdListO',hotkey:'Ctrl+O',title:'Ordered List',icon:{glyph:'glyphicon glyphicon-th-list',fa:'fa fa-list-ol','fa-3':'icon-list-ol'},callback:function(e){var chunk,cursor,selected=e.getSelection(),content=e.getContent()
if(selected.length==0){chunk=e.__localize('list text here')
...
cursor=selected.start-1}else{e.replaceSelection('`'+chunk+'`')
cursor=selected.start+1}
e.setSelection(cursor,cursor+chunk.length)}},{name:'cmdQuote',hotkey:'Ctrl+Q',title:'Quote',icon:{glyph:'glyphicon glyphicon-comment',fa:'fa fa-quote-left','fa-3':'icon-quote-left'},callback:function(e){var chunk,cursor,selected=e.getSelection(),content=e.getContent()
...
if(isPreview==false){e.showPreview()}else{e.hidePreview()}}}]}]],additionalButtons:[],reorderButtonGroups:[],hiddenButtons:[],disabledButtons:[],footer:'',onShow:function(e){},onPreview:function(e){},onSave:function(e){},onBlur:function(e){},onFocus:function(e){},onChange:function(e){}}
